      ...
      >
        ...
        {(title || showClose) && (
          <div className="flex items-center justify-between px-6 py-4 border-b border-[#e5eeff] bg-[#f8f9ff] shrink-0">
            {title && (
              <h3 className="text-xl font-bold text-[#0b1c30] tracking-tight truncate pr-2">{title}</h3>
            )}
            {showClose && (
              <button
                type="button"
                onClick={onClose}
                className="w-10 h-10 flex items-center justify-center rounded-xl text-[#7b7487] hover:bg-[#e5eeff] hover:text-[#0b1c30] active:scale-95 transition-all shrink-0 ml-auto"
